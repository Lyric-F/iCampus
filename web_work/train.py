...
# 注意：不再覆盖 cfg.LAMBDA_SSL 和 cfg.LAMBDA_NEG，使用 config.py 中的值
...
# 生成标签
# 直接从 interactions.csv 获取标签（假设顺序与 exposure_edges 一致）
df_inter_actions = df_inter['action'].str.lower().values
all_labels = torch.tensor([1 if act == 'like' else 0 for act in df_inter_actions], dtype=torch.float, device=cfg.DEVICE).unsqueeze(1)
