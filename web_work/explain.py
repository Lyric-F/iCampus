...
def explain_recommendation(user_id, top_k=5):
    ...
    # 获取用户历史交互的帖子索引（用于兴趣向量）
    hist_posts = user_to_posts.get(user_idx, [])
    if hist_posts:
        hist_emb = np.mean([post_embeddings[p] for p in hist_posts], axis=0)
    else:
        hist_emb = np.zeros(post_embeddings.shape[1])  # 维度 137
    ...
    # 计算帖子与用户历史兴趣的余弦相似度
    norm_hist = np.linalg.norm(hist_emb) + 1e-8
    sims = np.dot(post_embeddings, hist_emb) / (np.linalg.norm(post_embeddings, axis=1) * norm_hist)
    
    # 混合分数（调整权重，0.5 表示各一半）
    final_scores = 0.7 * scores + 0.3 * sims
