...
# ==================== 加速设置（不影响模型质量） ====================
# 启用 TF32（RTX 30/40 系支持，提速 1.2~1.5x）
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True

cfg = Config()

# 可选：验证频率（原每10轮，改为每20轮，减少验证开销）
VALIDATION_INTERVAL = 10

# 可选：启用 torch.compile（PyTorch 2.0+，首次运行编译后加速 10~30%）
USE_COMPILE = False   # 若需开启设为 True，需确保模型编译兼容
...
# ==================== 4. 构建训练样本（边 + 标签） ====================
exposure_edges = data["user", "interact", "post"].edge_index          # [2, E]
num_edges = exposure_edges.size(1)

action_to_idx = {"like": 0}
all_labels = []
for i in range(num_edges):
    ...
    # 单任务：1 表示 like，0 表示 view
    label = 1 if action == "like" else 0
    all_labels.append(label)
all_labels = torch.tensor(all_labels, dtype=torch.float, device=cfg.DEVICE).unsqueeze(1)   # [E, 1]


pos_tensor = data["user", "interact", "post"].pos               # [E]

perm = torch.randperm(num_edges, device=cfg.DEVICE)
val_size = int(cfg.VAL_RATIO * num_edges)
...
model = SGC_Rec(
    ...
    num_comments=len(comment_encoder.classes_),
...
for epoch in range(30):
    pos_indices = torch.randperm(len(train_user_ids))[:cfg.BATCH_SIZE * 10]
...
def graph_augmentation(edge_index_dict, drop_prob=0.2):
    new_dict = {}
    for key, edge_index in edge_index_dict.items():
        num_edges = edge_index.size(1)
        mask = torch.rand(num_edges, device=edge_index.device) > drop_prob
        new_dict[key] = edge_index[:, mask]
    return new_dict

def compute_contrastive_loss(emb1, emb2, temperature=0.5):
...
for epoch in range(cfg.EPOCHS):
    ...
    for i in range(0, len(perm), batch_size):
        ...
        pos_batch = train_pos[idx]
        seq_batch = train_seq[idx]
        seq_mask_batch = train_seq_mask[idx]

        with torch.no_grad():
            user_feat = data["user"].x[u_batch]
            post_feat = data["post"].x[p_batch]
            pos_feat = data["position"].x[pos_batch]
            p_score = propensity_model(user_feat, post_feat, pos_feat)

        with torch.cuda.amp.autocast(enabled=(cfg.DEVICE.type == 'cuda')):
            logits, x_dict = model(
                data.x_dict, data.edge_index_dict,
                u_batch, p_batch, seq_batch, seq_mask_batch, propensity=p_score
            )
            bce = F.binary_cross_entropy_with_logits(logits, y_batch, reduction='none')
            ipw_weights = 1.0 / (p_score + 1e-8).unsqueeze(1)
            ipw_weights = torch.clamp(ipw_weights, max=10.0)
            ipw_loss = (bce * ipw_weights).mean()
            residual = y_batch - torch.sigmoid(logits)
            dr_correction = (residual * (1.0 / p_score.unsqueeze(1) - 1.0)).mean()
            dr_loss = ipw_loss + dr_correction

            neg_user = torch.randint(0, data["user"].x.size(0), (len(u_batch) * cfg.NEG_SAMPLE_RATIO,), device=cfg.DEVICE)
            neg_post = torch.randint(0, data["post"].x.size(0), (len(u_batch) * cfg.NEG_SAMPLE_RATIO,), device=cfg.DEVICE)
            neg_seq = torch.full((len(u_batch) * cfg.NEG_SAMPLE_RATIO, cfg.SEQ_LEN), padding_idx, dtype=torch.long, device=cfg.DEVICE)
            neg_mask = torch.zeros(len(u_batch) * cfg.NEG_SAMPLE_RATIO, cfg.SEQ_LEN, dtype=torch.bool, device=cfg.DEVICE)
            neg_logits, _ = model(
                data.x_dict, data.edge_index_dict,
                neg_user, neg_post, neg_seq, neg_mask, propensity=None
            )
            neg_loss = F.binary_cross_entropy_with_logits(neg_logits, torch.zeros_like(neg_logits))

            edge_aug1 = graph_augmentation(data.edge_index_dict, cfg.GRAPH_AUG_DROP)
            edge_aug2 = graph_augmentation(data.edge_index_dict, cfg.GRAPH_AUG_DROP)
            x1 = model.graph_encoder(data.x_dict, edge_aug1)
            x2 = model.graph_encoder(data.x_dict, edge_aug2)
            ssl_loss = compute_contrastive_loss(x1["user"], x2["user"], cfg.TEMPERATURE)

            loss = dr_loss + cfg.LAMBDA_NEG * neg_loss + cfg.LAMBDA_SSL * ssl_loss
            loss = loss / accumulation_steps

        scaler.scale(loss).backward()
        if (i + batch_size) % (batch_size * accumulation_steps) == 0 or i + batch_size >= len(perm):
    ...
    if epoch % VALIDATION_INTERVAL == 0:
        ...
        with torch.no_grad():
            for i in range(0, len(val_user_ids), batch_size):
                ...
                pos_batch = val_pos[i:i+batch_size]
        ...
        print(f"Epoch {epoch}, loss={total_loss/len(perm)*batch_size:.4f}, val AUC={avg_auc:.4f}")
        ...
        if epoch % 50 == 0 and epoch > 0:
            for param_group in optimizer.param_groups:
                param_group['lr'] *= 0.5
                print(f"  -> Learning rate decreased to {param_group['lr']:.6f}")

# ==================== 9. 保存最终嵌入 ====================
model.load_state_dict(torch.load(cfg.BEST_MODEL))
model.eval()
with torch.no_grad():
    final_emb = model.graph_encoder(data.x_dict, data.edge_index_dict)
