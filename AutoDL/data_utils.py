...
def build_graph():
    ...
    post_emb = np.load(cfg.POST_EMB_NPY)          # 语义向量，形状 [num_posts, 256]
    ...
    # ----- 节点特征 -----
    # 用户特征：使用可训练嵌入的占位符（低维随机初始化，实际模型中将使用 nn.Embedding 替代）
    user_x = torch.randn(num_users, 16)          # 低维随机，仅为图消息传递提供入口

    # 帖子特征：语义向量 + 标签 one‑hot
    # 为每个帖子构建标签 one‑hot（取第一个标签）
    post_tag_onehot = []
    ...
    post_tag_onehot = torch.stack(post_tag_onehot, dim=0)          # [num_posts, num_tags]
    post_x = torch.cat([torch.tensor(post_emb, dtype=torch.float), post_tag_onehot], dim=1)
    print(f"帖子特征维度: {post_x.shape[1]}")   # 应为 256 + num_tags

    tag_x = torch.eye(num_tags, dtype=torch.float)                # 标签节点特征
    pos_x = torch.eye(num_positions, dtype=torch.float)           # 位置节点特征
    comment_x = torch.tensor(comment_emb, dtype=torch.float)      # 评论语义向量
    ...
    user_post_edges = torch.tensor(user_post_edges, dtype=torch.long).t().contiguous()
    edge_weights = torch.tensor(edge_weights, dtype=torch.float)
    edge_positions = torch.tensor(edge_positions, dtype=torch.long)
    ...
    for _, row in df_inter.iterrows():
        u = user_map[row["user_id"]]
        pos = pos_map[row["position"]]
        user_pos_edges.append([u, pos])
    user_pos_edges = torch.tensor(user_pos_edges, dtype=torch.long).t().contiguous()
    ...
    for p_id, tags in zip(df_posts["post_id"], df_posts["tags"].str.split(",")):
        ...
        for tag in tags:
            t = tag_map[tag]
            post_tag_edges.append([p, t])
    post_tag_edges = torch.tensor(post_tag_edges, dtype=torch.long).t().contiguous()
    ...
    for _, row in df_comments.iterrows():
        ...
        user_comment_edges.append([u, c])
    user_comment_edges = torch.tensor(user_comment_edges, dtype=torch.long).t().contiguous()
    ...
    for _, row in df_comments.iterrows():
        c = comment_map[row["comment_id"]]
        p = post_map[row["post_id"]]
        comment_post_edges.append([c, p])
    comment_post_edges = torch.tensor(comment_post_edges, dtype=torch.long).t().contiguous()

    # 用户-评论点赞边
    user_comment_like_edges = []
    for _, row in df_comment_likes.iterrows():
        u = user_map[row["user_id"]]
        c = comment_map[row["comment_id"]]
        user_comment_like_edges.append([u, c])
    user_comment_like_edges = torch.tensor(user_comment_like_edges, dtype=torch.long).t().contiguous()
