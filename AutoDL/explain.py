...
cfg = Config()
device = cfg.DEVICE
data = torch.load(cfg.GRAPH_PT, map_location=device)
...
def explain_recommendation(user_id, top_k=5):
    ...
    seq_tensor = torch.zeros(1, cfg.SEQ_LEN, dtype=torch.long, device=device)
    ...
    with torch.no_grad():
        ...
        scores = torch.sigmoid(logits[:, 0]).cpu().numpy()  # 以点赞任务分数排序
